...
class WGANGP(tfk.Model, GANBase):
    def __init__(
        ...
        # G tries to maximize D_Pg and D tries to minimize D_Pr and maximize D_Pr
        self.G_loss: Callable[[tf.Tensor], tf.Tensor] = lambda D_Pg: -tf.reduce_mean(
            D_Pg
        )
        self.D_loss: Callable[
            [tf.Tensor, tf.Tensor], tf.Tensor
        ] = lambda D_Pr, D_Pg: tf.reduce_mean(D_Pg) - tf.reduce_mean(D_Pr)
    ...
    def _get_GP(self, Pr, Pg):
        ...
        # so D_pred_Prg = D(Prg) and we need dD(Prg)/dPrg (and not dD_w/dw which is typically used for grad. descent!)
    ...
    def train_step(self, Pr):
        ...
        for _ in range(self.D_n):  # TODO: create convergence option
            ...
            # we separately forward propagate Pr and Pg so that we can easily compute
            # the 1st and 2nd term of the first part of Eq. 3
            with tf.GradientTape() as g_tape:
        ...
        # Sidenote after convergence D should approximate the one Lipschitz continous function
        # within the Wasserstein Distance formulation
        ...
        # Compute set of relevant losses
        current_losses = {}
        current_losses["Wasserstein distance"] = tf.abs(
            self.D_loss(D_pred_Pr, D_pred_Pg)
        )
        current_losses["D_loss_Pr"] = tf.reduce_mean(self.D(Pr))
        current_losses["D_loss_Pg"] = tf.reduce_mean(self.D(Pg))
